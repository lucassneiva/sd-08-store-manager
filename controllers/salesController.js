...
const createSale = async (req, res) => {
  const sale = req.body;
  const validation = middle.validateSale(sale);
  if (validation) {
    res.status(error).json(validation);
  }
  const result = await salesService.createSale(sale);
  const test = await middle.saleQuantity(sale, negative);
  if (test) {
...
const updateSale = async (req, res) => {
  ...
  await middle.saleQuantity(oldSale.itensSold, 1);
  await middle.saleQuantity(sale, negative);
  res.status(ok).json(result);
};
...
const deleteSale = async (req, res) => {
  ...
  await middle.saleQuantity(sale.itensSold, 1);
  res.status(ok).json(result);
};
