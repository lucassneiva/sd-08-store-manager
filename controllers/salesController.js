...
router.get('/', rescue(async (_req, res) => {
  ...
  if(!sales) return res.status(StatusCodes.NOT_FOUND).send({ message: 'Not found' });
...
router.get('/:id', rescue(async (req, res) => {
  ...
  if (message) return res.status(StatusCodes.NOT_FOUND).json({
    err: { 
      code: 'not_found',
  ...
  if (!sales) return res.status(StatusCodes.NOT_FOUND).json({
    err: { 
      code: 'not_found',
      message: 'Sale not found'
    }
...
router.delete('/:id', validateSalesId, rescue(async (req, res) => {
  ...
  return res.status(StatusCodes.OK).send(saleToDelete);
}));
...
router.post('/', validateSales, rescue(async (req, res) => {
  ...
  if (!err) return res.status(StatusCodes.OK).send({ _id, itensSold });
  ...
  // return res.status(StatusCodes.OK).send({ _id, itensSold });
}));
